...
Ebookr.prototype.cli = function () {
	var args = require('./ebookr/cli');
	if (args.version) {
		console.log('ebookr v' + this.version());
		return;
	}
	if (args.metadata) {
		args.metadata.forEach(function (m) {
			var keyAndValue = m.split('=');
			this.metadata(keyAndValue[0], keyAndValue[1] || true);
		}.bind(this));
	}
	if (args.files.length > 0) {
		this.convertFile(args.files, args);
	} else {
...
Ebookr.prototype.metadata = require('./ebookr/metadata');
Ebookr.prototype.new = function () {
	return new Ebookr();
};
Ebookr.prototype.pandoc = function () {
		return pandoc.convert.apply(this, arguments);
};
