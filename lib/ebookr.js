...
Ebookr.prototype.convert = function (srcFile, outFile) {
	var self = this;
	fs.readFile(srcFile, this.options.encoding, function (err, data) {
		var renderedText = self.parse(data).render();
		fs.writeFile('tmp.md', renderedText, function (err) {
			var options = [];
			if (outFile) {
			...
			var cmd = util.format('pandoc tmp.md %s', options.join(' '));
			shell.exec(cmd);
			fs.unlink('tmp.md');
		});
	});
